...
```bash
shared-connection$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
shared-connection$ sam local start-api
shared-connection$ curl http://localhost:3000/
```
...
```bash
shared-connection$ sam logs -n HelloWorldFunction --stack-name shared-connection --tail
```
...
```bash
shared-connection$ cd hello-world
...
```bash
sam delete --stack-name shared-connection
```
